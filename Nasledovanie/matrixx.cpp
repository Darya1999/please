...
int Matrixx::get(int i, int j)
{
	if ((matrix != 0) && (i < n) && (j < m))
	{
		return matrix[i * m + j]; // чтение элемента на позиции [i, j] 
	}
	else
	{
		return 0;
...
void Matrixx::set(int i, int j, int value)
{
	if ((matrix != 0) && (i < n) && (j < m))
	{
		matrix[i * m + j] = value; // запись элемента на позицию [i, j]
	}
}
...
void Matrixx::print() // dsdjl матрицы 
{
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < m; j++)
...
void Matrixx::matrix_matrix(int **result, int **matrix1, int **matrix2, int n1, int m1, int n2, int m2) //подсчет суммы матриц 
{
